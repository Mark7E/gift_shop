...
const Cart = (props) => {
    ...
    const qtyPlus = (id, qty) => {

        setQty(qty += 1)
        console.log(qty)
        console.log(id)

        axios.put(`http://localhost:8000/api/cart/${id}`, { qty })
            .then(res => {
                history.push("/cart")
            })
            .catch(err => console.log(err))
        ...
        cart.length > 0 ?
            <div>
                <div className="myheader">
                    <h1 onClick={routeChange} className='pointer'> Gifts Shop </h1>
                    <Link to={`/cart`}>
                ...
                <div className="singleview">
                    ...
                    <div className="">
                                    ...
                                    return (
                                        <div className="product" key={item._id} >
                                            <div>
                                                ...
                                                <button className="btn btn-3" onClick={(e) => deleteP(item._id)} >Remove </button>
                                            </div>
                                        </div>
                                    )
                                }
                        ...
                        <button className="btn btn-2" onClick={routeChangeChkout}>Check out</button>
                    </div>
                </div>
            </div >
